...
namespace RaidLog.Controllers
{
    ...
    public class ProjectsController : RaidLogApiController
    {
            ...
            try
            {
                using (var tx = _connection.BeginTransaction(IsolationLevel.ReadCommitted))
                {
                    ...
                    var projectDependencyStatuses = result.Read<ProjectAndDependencyStatus>()
                                                          .ToArray();
                    var projectQueryStatuses = result.Read<ProjectAndQueryStatus>()
                                                     .ToArray();
        ...
        private void SetRiskCounts(IDictionary<int, ProjectSummaryWithCounts> projectSummaries, ProjectAndRiskStatus[] projectRiskStatuses, ProjectAndAssumptionStatus[] projectAssumptionStatuses, ProjectAndIssueStatus[] projectIssuesStatuses, ProjectAndDependencyStatus[] projectDependencyStatuses, ProjectAndQueryStatus[] projectQueryStatuses, ProjectAndActionStatus[] projectActionsStatuses)
        {
            ...
            foreach (var queryStatus in projectQueryStatuses)
            {
                var psc = projectSummaries[queryStatus.ProjectId];
                if (queryStatus.IsActive)
                {
                    psc.ActiveQueries++;
                }
                else
